    ...
    for (let id in props.event.events) {
        ...
        eventsArr.push(
            <tr key={id} onClick={(e) => {
                ...
                <td>
                    Наставник: {props.users[props.mentoringConnections[props.event.events[id].table_id]?.mentor_id]?.full_name}<br />
                    Стажер: {props.users[props.mentoringConnections[props.event.events[id].table_id]?.protege_id]?.full_name}
                </td>
                <td></td>
                <td>{createDate}</td>
                <td></td>
...
export default withRouter(MentoringEvent);